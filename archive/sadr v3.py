...
def Show_Reader_Book_Options():
    ...
    login_button.pack(padx=10, ipady=25, ipadx=50)
    login_button = tk.Button(window, text="return the book", command=SearchBookByAuthorName, bg="orange")
    login_button.pack(padx=10, ipady=25, ipadx=50)
    login_button = tk.Button(window, text="Submit an extension request", command=SearchBookByAuthorName, bg="orange")
    login_button.pack(padx=10, ipady=25, ipadx=50)
    login_button = tk.Button(window, text="back page", command=ReadersMainPage)
    login_button.pack(padx=10, ipady=25, ipadx=50)
...
def SearchBookByName_(name: str):
    ...
    selected_books = cursor.execute(query).fetchmany()
    conn.commit()
    if selected_books:
        username_label = tk.Label(window, text=f'ID,Title,Author,ISBN,PublishedDate,Gener,AvailableCopy')
        username_label.pack(pady=10, ipadx=290)
        username_label = tk.Label(window, text=f'{selected_books}')
        username_label.pack(pady=10, ipadx=290)
        search_label = tk.Label(window, text='enter books id that you want')
        search_label.pack(pady=10)
        search_field = tk.Entry(window)
        search_field.pack(pady=10)
        search_button = tk.Button(window, text="receive", command=lambda: bookloan(search_field.get()),
                                  bg="orange")
        search_button.pack(padx=10, ipady=25, ipadx=50)
    else:
        search_label = tk.Label(window, text='There is no book available with this information')
        search_label.pack(pady=10)
    login_button = tk.Button(window, text="back page", command=Show_Reader_Book_Options, bg="green")
    login_button.pack(padx=10, ipady=25, ipadx=50)


def bookloan(id_book):
    sql_insert = "INSERT INTO Transactions (MemberID,BookID) VALUES (?, ?) "
    values = (ID, id_book)
    cursor.execute(sql_insert, values)
    conn.commit()
    messagebox.showinfo("Adding book", f"Book added successfully ID:{ID} ")

    select_query = f"SELECT AvailableCopies FROM books WHERE BookID = {id_book}"
    result = cursor.execute(select_query).fetchone()
    conn.commit()
    bookcount = int(result[0]) - 1

    select_query = f"UPDATE books SET AvailableCopies = {bookcount} WHERE BookID = {id_book}"
...
def SearchBookByAuthorName_(name: str):
    ...
    selected_books = cursor.execute(query).fetchall()
    conn.commit()

    if selected_books:
        username_label = tk.Label(window,
                                  text=f' ID   ,    Title,   Author,   ISBN,   PublishedDate,   Gener,   AvailableCopy')
        username_label.pack(pady=10, ipadx=290)
...
def AddBook_(booktitle, bookAuthor, bookISBN, bookPublishDate, bookGenre, bookAvailableCopies):
    ...
    values = (
        booktitle, bookAuthor, bookISBN, bookPublishDate, bookGenre, bookAvailableCopies)
...
def DeleteBook_(DeletingBookId):
    ...
    query = f'SELECT * FROM Books WHERE BookID={DeletingBookId}'
    result = cursor.execute(query).fetchmany()
    if not result:
        messagebox.showwarning('not found', 'no book with this id')
        return
    query = f'DELETE FROM books WHERE BookID={DeletingBookId}'
    cursor.execute(query)
    conn.commit()
    messagebox.showinfo('success', f'member with id={ID} deleted successfully')
