...
def reset_page(x=500, y=600):
    global window
    if window:
        window.destroy()
    window = tk.Tk()
    window.geometry(f"{x}x{y}+500+100")
...
def Show_Reader_Book_Options():
    ...
    login_button.pack(padx=10, ipady=25, ipadx=50)
    login_button = tk.Button(window, text='Owned books', command=ShowOwnedBooks, bg='orange')
    login_button.pack(padx=10, ipady=25, ipadx=50)
...
def SearchBookByName_(name: str):
    global conn
    global cursor
    query = f"SELECT * FROM Books WHERE Title like '%{name}%' AND AvailableCopies > 0"
...
def SearchBookByAuthorName_(name: str):
    global conn
    global cursor
    query = f"SELECT * FROM Books WHERE Author = '%{name}%' AND AvailableCopies > 0"
    selected_books = cursor.execute(query)
    conn.commit()
    ShowBooks(selected_books)
...
def SearchBookByGenre_(genre: str):
    global conn
    global cursor
    query = f"SELECT * FROM Books WHERE Genre = '{genre}' AND AvailableCopies > 0"
    selected_books = cursor.execute(query)
    conn.commit()
    ShowBooks(selected_books)
...
def ShowOwnedBooks():
    ...
    global ID
    global window
    query = f'select * from Books where BookID in (select BookID from Transactions where MemberID={ID})'
    result = cursor.execute(query)
    ShowBooks(result,  searchNotPersonal=False)


def ShowBooks(items: list,  searchNotPersonal=True):
    reset_page(600, 700)
    items = items.fetchall()
    ShowBooks_(items, window, cursor, conn, ID, Show_Reader_Book_Options, searchNotPersonal=searchNotPersonal)
...
def StaffMainPage():
    ...
    username_label = tk.Label(window, text="If you have not registered yet, click on the button below")
    username_label.pack(pady=10, ipadx=290)
    Signin_button = tk.Button(window, text="Sign In", command=SigninStaff, bg="Yellow")
    Signin_button.pack(pady=10, ipady=30, ipadx=30)
    login_button = tk.Button(window, text="Back Page", command=first_page)
    login_button.pack(padx=13, ipady=10, ipadx=20)
...
def CheckLoginINFOStaff(firstname, lastname, phonenumber, email):
    ...
    if is_valid_name(firstname) and is_valid_name(lastname) and \
        ...
        ID = cursor.execute(query).fetchval()
        messagebox.showinfo("you may now login to your library", 'your ID is:' + str(ID))
        ShowStaffPage(ID, phonenumber)
    else:
        messagebox.showinfo('Login Error', 'wrong information format')
        LoginStaff()
...
def DeleteMember():
    ...
    submit = tk.Button(window, text="Delete", command=lambda: [DeleteMember_(id_entry.get()), Show_Librarian_Options()])
    submit.pack(pady=10)
...
def AddBook():
    reset_page()
    window.title("Adding Book")
    booktitle_label = tk.Label(window, text="booktitle:", bg='green')
    booktitle_label.pack(pady=10)
    booktitle_entry = tk.Entry(window)
    booktitle_entry.pack(pady=5)

    bookAuthor_label = tk.Label(window, text="bookAuthor:", bg='green')
    bookAuthor_label.pack(pady=10)
    bookAuthor_entry = tk.Entry(window)
    bookAuthor_entry.pack(pady=5)

    bookISBN_label = tk.Label(window, text="bookISBN:", bg='green')
    bookISBN_label.pack(pady=10)
    bookISBN_entry = tk.Entry(window)
    bookISBN_entry.pack(pady=5)

    bookPublishDate_label = tk.Label(window, text="bookPublishDate:", bg='green')
    bookPublishDate_label.pack(pady=10)
    bookPublishDate_entry = tk.Entry(window)
    bookPublishDate_entry.pack(pady=5)

    bookGenre_label = tk.Label(window, text="bookGenre:", bg='green')
    bookGenre_label.pack(pady=10)
    bookGenre_entry = tk.Entry(window)
    bookGenre_entry.pack(pady=5)

    bookAvailableCopies_label = tk.Label(window, text="bookAvailableCopies:", bg='green')
    bookAvailableCopies_label.pack(pady=10)
    bookAvailableCopies_entry = tk.Entry(window)
    bookAvailableCopies_entry.pack(pady=5)

    login_button = tk.Button(window, text="AddingBook",
                             command=lambda: AddBook_(booktitle_entry.get(), bookAuthor_entry.get(),
                                                      bookISBN_entry.get(),
                                                      bookPublishDate_entry.get(), bookGenre_entry.get(),
                                                      bookAvailableCopies_entry.get()))
    login_button.pack(pady=10)

    login_button = tk.Button(window, text="Back Page", command=Show_Librarian_Options, bg='orange')
    login_button.pack(padx=13, ipady=10, ipadx=20)


def AddBook_(booktitle, bookAuthor, bookISBN, bookPublishDate, bookGenre, bookAvailableCopies):
    sql_insert = "INSERT INTO Books (Title, Author, ISBN, PublishDate ,Genre,AvailableCopies) VALUES (?, ?, ?,?,?,?) "
    values = (booktitle, bookAuthor, bookISBN, bookPublishDate, bookGenre, bookAvailableCopies)
    cursor.execute(sql_insert, values)
    conn.commit()
    messagebox.showinfo("Adding book", f"Book added successfully name:{booktitle} ")


def DeleteBook():
    reset_page()
    DeletingBookId_label = tk.Label(window, text="Deleting BookId:")
    DeletingBookId_label.pack(pady=10)
    DeletingBookId_entry = tk.Entry(window)
    DeletingBookId_entry.pack(pady=5)

    login_button = tk.Button(window, text="Delete book", command=lambda: DeleteBook_(DeletingBookId_entry.get()),
                             bg="red")
    login_button.pack(padx=10, ipady=25, ipadx=50)
    login_button = tk.Button(window, text="Back Page", command=Show_Librarian_Options)
    login_button.pack(padx=13, ipady=10, ipadx=20)
...
def DeleteBook_(DeletingBookId):
    global conn
    global cursor
    if type(DeletingBookId) == str: id = int(DeletingBookId)
    query = f'SELECT * FROM Books WHERE BookID={id}'
    result = cursor.execute(query).fetchmany()
    if not result:
        messagebox.showwarning('not found', 'no book with this id')
        return
    query = f'DELETE FROM Books WHERE BookID={id}'
    cursor.execute(query)
    conn.commit()
    messagebox.showinfo('success', f'book with id={id} deleted successfully')
